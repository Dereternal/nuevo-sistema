...
export function AddUnitModal({ onClose, onSuccess }: AddUnitModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase
        .from('units')
        .insert({ name: name.trim(), abbreviation: abbreviation.trim(), created_by: userData.user.id })
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold text-gray-800">Nueva Unidad de Medida</h3>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <input
            ...
            placeholder="Abreviatura (Ej: KG, LT, UND)"
